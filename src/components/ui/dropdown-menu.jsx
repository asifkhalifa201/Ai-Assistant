import React, { useState, useContext, cloneElement, useRef, useEffect } from 'react';
...
export function DropdownMenu({ children }) {
  ...
  return (
    <DropdownMenuContext.Provider value={{ open, setOpen }}>
      <div ref={containerRef} className="relative inline-flex">{children}</div>
...
export function DropdownMenuContent({ children, className = '' }) {
  ...
  return (
    <div className={`absolute right-0 top-full z-50 mt-2 w-44 rounded-xl border border-zinc-200 bg-white p-1 shadow-xl ${className}`}>
      {children}
    </div>
  );
}
